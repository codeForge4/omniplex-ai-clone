    ...
    return [
      {
        ...
        content:
          ...
          "You have the ability to search and will be given websites and the scraped data from them and you will have to make up an answer with that only" +
...
export const cutString = (str: string, maxLength: number = 50) => {
  ...
  if (trimmedStr.lastIndexOf(" ") > 0) {
    trimmedStr = trimmedStr.slice(0, Math.min(trimmedStr.length, trimmedStr.lastIndexOf(" ")));
  }
  return trimmedStr + "...";
};
